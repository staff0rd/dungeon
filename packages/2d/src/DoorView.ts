...
export class DoorView {
  ...
  drawPattern() {
    ...
    if (this.plane == Plane.Vertical) container.rotation = Math.PI / 2;

    container.pivot.set(container.width / 2, container.height / 2);
    container.position.set(
      this.scale * (this.position.x + 0.5),
      this.scale * (this.position.y + 0.5)
    );
  ...
  private getDoorHandle() {
    ...
    handle.beginFill(Colors.Black).drawCircle(0, 0, 2).endFill();
    return handle;
  }
  ...
  drawNumber() {
    const g = new PIXI.Text(this.number.toString(), {
      fontSize: 12,
    ...
    g.position.set(
      (this.position.x + 0.5) * this.scale,
      (this.position.y + 0.5) * this.scale
    );
  }
