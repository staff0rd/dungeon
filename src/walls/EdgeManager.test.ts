...
describe ("EdgeManager", () => {
    ...
    describe('join', () => {
        ...
        test('should retain splits', () => {
            const edge1 = manager.getCorridorEdge(<CorridorView>{ rect: new Rect(5, 5, 10, 1) }, [], [new Rect(6, 4, 1, 1)], Direction.Top);
            expect(edge1.segments.length).toBe(2);
            const edge2 = manager.getCorridorEdge(<CorridorView>{ rect: new Rect(4, 5, 1, 1)}, [], [], Direction.Top);
    ...
    describe('getCorridorEdge', () => {
        test('should split by adjacent corridors', () => {
            const edge = manager.getCorridorEdge(<CorridorView>{ rect: new Rect(5, 5, 10, 1) }, [], [new Rect(6, 4, 1, 1)], Direction.Top);
...
function getMapEdges(seed: number, direction: Direction, selectEdge: (r: Rect) => boolean, width = 60, height = 30) {
