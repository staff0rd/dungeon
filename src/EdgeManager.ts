...
export class EdgeManager {
  ...
  getCorridorEdge(
    ...
    const corridorsToCheckIntersect = corridorRects.filter(
      (c) => !corridor.rect.equals(c)
    );
    const rects = rooms
      .map((r) => r.rect)
      .concat([].concat(...doors))
  ...
  getCorridorEdges(
    ...
    let edges = corridors.map((corridor) =>
      this.getCorridorEdge(
        corridor,
        rooms,
  ...
  private getEdge(baseRect: Rect, rects: Rect[], direction: Direction) {
    ...
    switch (direction) {
      ...
      case Direction.Left: {
        ...
        edgeInserter = (rect: Rect) =>
          edge.insert(rect.y1, rect.y2, Structure.Corridor);
        intersections = rects.filter((rect) =>
          overlap(rect.y1, rect.y2, baseRect.y1, baseRect.y2)
        );
        break;
      }
      case Direction.Top:
      case Direction.Bottom: {
        edge = new Edge(baseRect, direction, Structure.Room);
        edgeInserter = (rect: Rect) =>
          edge.insert(rect.x1, rect.x2, Structure.Corridor);
        intersections = rects.filter((rect) =>
          overlap(baseRect.x1, baseRect.x2, rect.x1, rect.x2)
        );
        break;
      }
    }
  ...
  join(edges: Edge[]): Edge[] {
    ...
    for (let i = 0; i < nonEmptyEdges.length; i++) {
      ...
      const aligned = notEqual.filter((e) =>
        e.rect.aligned(first.rect, first.direction)
      );
